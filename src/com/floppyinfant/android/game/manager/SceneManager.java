...
import com.floppyinfant.android.game.scenes.layers.OptionsLayer;
...
public class SceneManager {
	...
	private int mNumFramesPassed = -1;	// Used by the mLoadingScreenHandler, this variable ensures that the loading screen is shown for one frame prior to loading resources.
	private boolean mLoadingScreenHandlerRegistered = false;	// Keeps the mLoadingScreenHandler from being registered with the engine if it has already been registered.
	...
	private boolean mCameraHadHud = false;	// Set to TRUE in the showLayer() method if the camera had a HUD before the layer was shown.
	private Scene mPlaceholderModalScene;	// An empty place-holder scene that we use to apply the modal properties of the layer to the currently shown scene.
	...
	public void showLayer(final ManagedLayer pLayer, final boolean pSuspendSceneDrawing, final boolean pSuspendSceneUpdates, final boolean pSuspendSceneTouchEvents) {
		...
		// TODO Scaling
		pLayer.setScale(ResourceManager.getInstance().cameraScaleX, ResourceManager.getInstance().cameraScaleY);
